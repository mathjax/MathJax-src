...
export class Menu {
  ...
  protected initSettings() {
    ...
    this.settings.overflow =
      jax.options.displayOverflow.substr(0, 1).toUpperCase() +
      jax.options.displayOverflow.substr(1).toLowerCase();
  ...
  protected initMenu() {
    ...
    this.menu = parser.parse({
      ...
      pool: [
        ...
        this.variable<boolean>('explorer', (explore) =>
          this.setExplorer(explore),
        ),
  ...
  protected checkLoadableItems() {
    if (MathJax && MathJax._ && MathJax.loader && MathJax.startup) {
      if (
        ...
        (!MathJax._.a11y || !MathJax._.a11y.complexity)
      ) {
        this.loadA11y('complexity');
      }
      if (
        this.settings.explorer &&
        (!MathJax._.a11y || !MathJax._.a11y.explorer)
      ) {
        this.loadA11y('explorer');
      }
      if (
        this.settings.assistiveMml &&
        (!MathJax._.a11y || !MathJax._.a11y['assistive-mml'])
      ) {
  ...
  protected enableExplorerItems(enable: boolean) {
    const menu = (this.menu.findID('Accessibility', 'Activate') as Submenu)
      .menu;
  ...
  protected applySettings() {
    ...
    promise.then(() => {
      ...
      this.document.outputJax.options.displayOverflow =
        this.settings.overflow.toLowerCase();
      this.document.outputJax.options.linebreaks.inline =
  ...
  protected setOutputJax(jax: string, rerender: boolean = true): Promise<void> {
    ...
    return rerender
      ? mathjax.handleRetriesFor(() => this.rerender())
      : Promise.resolve();
  }
  ...
  protected setExplorer(explore: boolean) {
    ...
    if (!explore || (MathJax._.a11y && MathJax._.a11y.explorer)) {
      this.rerender(
        this.settings.collapsible ? STATE.RERENDER : STATE.COMPILED,
      );
    } else {
      this.loadA11y('explorer');
    }
  ...
  protected toMML(math: HTMLMATHITEM): string {
    ...
    return !this.settings.filterSRE
      ? mml
      : mml.replace(
          / (?:data-semantic-.*?|role|aria-(?:level|posinset|setsize))=".*?"/g,
          '',
        );
  }
    ...
    if (
      ...
      for (const child of adaptor.childNodes(math.typesetRoot)) {
        if (adaptor.kind(child) === 'svg') {
          return Promise.resolve(
            this.formatSvg(adaptor.outerHTML(child as HTMLElement)),
          );
        }
      }
    }
