...
describe('Newcommand', () => {
  ...
  it('Let Self', () => {
    expectTexError('\\let\\x\\x \\x').toBe('Undefined control sequence \\x');
  });
  ...
  it('Def Template Matching', () => {
    expect(
      tex2mml(
        '\\def\\ending{+}\\def\\test#1\\end{[#1]} \\test a\\ending b\\end'
      )
    ).toMatchSnapshot();
  });
...
describe('NewcommandError', () => {
  ...
  it('No Sequence', () => {
    expectTexError('\\def\\bar[#1]#3{}').toBe(
      'Parameters for \\bar must be numbered sequentially'
    );
  });

  it('No CS', () => {
    expectTexError('\\def{\\bar}[#1]#2{}').toBe(
      '\\def must be followed by a control sequence'
    );
  });

  it('Illegal Hash', () => {
    expectTexError('\\def\\bar[##1]#2{#1}').toBe(
      'Illegal use of # in template for \\bar'
    );
  });

  it('No Replacement', () => {
    expectTexError('\\def\\bar[#1]#2').toBe(
      'Missing replacement string for definition of \\def'
    );
  });

  it('Runaway Argument', () => {
    expectTexError('\\def\\bar[#1]#2{}\\bar[').toBe(
      'Runaway argument for \\bar?'
    );
  });
  ...
  it('Illegal Parameter Number', () => {
    expectTexError('\\newenvironment{hh}[a]{}{}').toBe(
      'Illegal number of parameters specified in \\newenvironment'
    );
  });

  it('Let Undefined CS', () => {
    expectTexError('\\let\\aa\\bb \\aa').toBe(
      'Undefined control sequence \\aa'
    );
  });

  it('Missing Arguments', () => {
    expectTexError('\\def\\bar[#1]#2#3{c + c}\\bar').toBe(
      "Use of \\bar doesn't match its definition"
    );
  });

  it('Single Let Error', () => {
    expectTexError('\\let\\aa\\textbf\\let\\bb\\aa\\aa').toBe(
      'Missing argument for \\aa'
    );
  });

  it('Double Let Error', () => {
    expectTexError('\\let\\aa\\textbf\\let\\bb\\aa\\bb').toBe(
      'Missing argument for \\bb'
    );
  });
  ...
  it('Illegal Argument Number', () => {
    expectTexError('\\newcommand{\\foo}[a]{#1 + #2}').toBe(
      'Illegal number of parameters specified in \\newcommand'
    );
  });

  it('Optional Brace Error', () => {
    expectTexError('\\def\\bar[{#1}]#2{#1 + #2}').toBe(
      "You can't use 'macro parameter character #' in math mode"
    );
  });

  it('Missing End Error', () => {
    expectTexError('\\newenvironment{env}{aa}{bb}\\begin{env}cc').toBe(
      'Missing \\end{env}'
    );
  });

  it('Hash Error', () => {
    expectTexError('\\def\\x#1{a #1 b #c} \\x{a}').toBe(
...
describe('Newcommand Overrides', () => {
  ...
  it('Let def macro be undefined', () => {
    expectTexError('\\def\\test{error} \\let\\test=\\undefined \\test').toBe(
      'Undefined control sequence \\test'
    );
  });

  it('Let existing macro be undefined', () => {
    expectTexError('\\let\\sqrt=\\undefined \\sqrt{x}').toBe(
      'Undefined control sequence \\sqrt'
    );
  });

  it('Let existing delimiter be undefined', () => {
    expectTexError('\\let\\|=\\undefined \\left\\| X \\right\\|').toBe(
      'Missing or unrecognized delimiter for \\left'
    );
  });

  it('Let after def of existing macro be undefined', () => {
    expectTexError('\\def\\sqrt{X} \\let\\sqrt=\\undefined \\sqrt{x}').toBe(
      'Undefined control sequence \\sqrt'
    );
  });
  ...
  it('Def overrides let delimiter', () => {
    expectTexError(
      '\\let\\test=\\| \\def\\test{x} \\left\\test X \\right\\test'
  ...
  it('Def overrides existing delimiter', () => {
    expectTexError('\\def\\|{x} \\left\\| X \\right\\|').toBe(
      'Missing or unrecognized delimiter for \\left'
    );
  });
  ...
  it('Let overrides delimiter', () => {
    expectTexError('\\let\\|=\\sqrt \\left\\| X \\right\\|').toBe(
      'Missing or unrecognized delimiter for \\left'
    );
  });
  ...
  it('Let of character macro overrides delimiter', () => {
    expectTexError('\\let\\|=\\alpha \\left\\| X \\right\\|').toBe(
      'Missing or unrecognized delimiter for \\left'
    );
  });
...
describe('Nested Environments', () => {
  ...
  it('Ended by Wrong Environment', () => {
    expectTexError('\\newenvironment{a}{x}{y}\\begin{a} X \\end{cases}').toBe(
      '\\begin{a} ended with \\end{cases}'
    );
  });
