...
/**
 * @fileoverview  The webpack configuration file for use with ES-modules
 *
...
const locale = (process.argv[6] === '--env' ? process.argv[7].split(/=/)[1] : 'en');
const json = require(path.join(dir, 'config.json')).webpack;

let pkg = PACKAGE({...json, target: 'mjs', bundle, dir, locale});
